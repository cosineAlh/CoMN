...
def acq_max(ac, gp, y_max, bounds, random_state, n_warmup=10000, n_iter=10):
    ...
    for x_try in x_seeds:
        ...
        res = scipy.optimize.minimize(lambda x: -ac(x.reshape(1, -1), gp=gp, y_max=y_max),
                       x_try.reshape(1, -1).flatten(),
                       bounds=bounds,
                       method="L-BFGS-B")
        ...
        # res_fun = float(str(str(str(res).split('[')[1]).split(']')[0]))
        res_fun = res.fun
        if max_acq is None or -1*res_fun >= max_acq:
            x_max = res.x
            max_acq = -1*res_fun
        # x_max = 0
        # max_acq = 1
